...
local function pre_process(msg)
	...
	if not warn_max then
		data[tostring(msg.to.id)]['settings']['warn_max'] = 10
...
local function unwarn_user(user_id, chat_id)
    ...
    if tonumber(warns) <= 0 then
        ...
        send_large_msg(chat, 'One warn has been deleted, keep Calm!', ok_cb, false)
        send_large_msg(channel, 'One warn has been deleted, keep Calm!', ok_cb, false)
    end
end
---------------------------------------------------------------
...
local function Warn_by_username(extra, success, result)
    if success == 0 then
        return send_large_msg(receiver, 'Can\'t find this username.')
    end
